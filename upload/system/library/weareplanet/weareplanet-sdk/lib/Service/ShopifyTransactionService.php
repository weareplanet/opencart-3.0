...
class ShopifyTransactionService {
	...
	public function readWithHttpInfo($space_id, $id) {
		...
		try {
			...
			$response = $this->apiClient->callApi(
				...
				$httpBody,
				$headerParams,
				'\WeArePlanet\Sdk\Model\ShopifyTransaction',
				'/shopify-transaction/read'
			);
			return new ApiResponse($response->getStatusCode(), $response->getHeaders(), $this->apiClient->getSerializer()->deserialize($response->getData(), '\WeArePlanet\Sdk\Model\ShopifyTransaction', $response->getHeaders()));
		} catch (ApiException $e) {
			switch ($e->getCode()) {
                case 200:
                    $data = ObjectSerializer::deserialize(
	 ...
	 *
	 * @param int $space_id  (required)
	 * @param \WeArePlanet\Sdk\Model\EntityQuery $query The query restricts the Shopify transactions which are returned by the search. (required)
	 * @throws \WeArePlanet\Sdk\ApiException
	 * @throws \WeArePlanet\Sdk\VersioningException
	 * @throws \WeArePlanet\Sdk\Http\ConnectionException
	 * @return \WeArePlanet\Sdk\Model\ShopifyTransaction[]
