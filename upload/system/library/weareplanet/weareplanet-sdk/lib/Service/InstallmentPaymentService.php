...
class InstallmentPaymentService {
	 ...
	 * @param \WeArePlanet\Sdk\Model\EntityQueryFilter $filter The filter which restricts the installment payment which are used to calculate the count. (required)
	 ...
	 * @throws \WeArePlanet\Sdk\VersioningException
	 * @throws \WeArePlanet\Sdk\Http\ConnectionException
	 * @return \WeArePlanet\Sdk\Model\InstallmentPayment
	 */
	...
	public function createInstallmentPaymentWithHttpInfo($space_id, $transaction_id, $installment_plan_configuration) {
		...
		try {
			...
			$response = $this->apiClient->callApi(
				...
				$httpBody,
				$headerParams,
				'\WeArePlanet\Sdk\Model\InstallmentPayment',
				'/installment-payment/createInstallmentPayment'
			);
			return new ApiResponse($response->getStatusCode(), $response->getHeaders(), $this->apiClient->getSerializer()->deserialize($response->getData(), '\WeArePlanet\Sdk\Model\InstallmentPayment', $response->getHeaders()));
		} catch (ApiException $e) {
			switch ($e->getCode()) {
                case 200:
                    $data = ObjectSerializer::deserialize(
	 ...
	 *
	 * @param int $space_id  (required)
	 * @param \WeArePlanet\Sdk\Model\EntityQuery $query The query restricts the installment payments which are returned by the search. (required)
	 * @throws \WeArePlanet\Sdk\ApiException
	 * @throws \WeArePlanet\Sdk\VersioningException
	 * @throws \WeArePlanet\Sdk\Http\ConnectionException
	 * @return \WeArePlanet\Sdk\Model\InstallmentPayment[]
