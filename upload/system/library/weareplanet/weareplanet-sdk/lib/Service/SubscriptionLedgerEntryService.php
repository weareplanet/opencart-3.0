...
class SubscriptionLedgerEntryService {
	 ...
	 *
	 * @param int $space_id  (required)
	 * @param \WeArePlanet\Sdk\Model\SubscriptionLedgerEntryCreate $entry  (required)
	 * @throws \WeArePlanet\Sdk\ApiException
	 * @throws \WeArePlanet\Sdk\VersioningException
	 * @throws \WeArePlanet\Sdk\Http\ConnectionException
	...
	public function createWithHttpInfo($space_id, $entry) {
		...
		try {
			...
			$response = $this->apiClient->callApi(
				...
				$httpBody,
				$headerParams,
				'\WeArePlanet\Sdk\Model\SubscriptionLedgerEntry',
				'/subscription-ledger-entry/create'
			);
			return new ApiResponse($response->getStatusCode(), $response->getHeaders(), $this->apiClient->getSerializer()->deserialize($response->getData(), '\WeArePlanet\Sdk\Model\SubscriptionLedgerEntry', $response->getHeaders()));
		} catch (ApiException $e) {
			switch ($e->getCode()) {
                case 200:
                    $data = ObjectSerializer::deserialize(
	 ...
	 *
	 * @param int $space_id  (required)
	 * @param \WeArePlanet\Sdk\Model\EntityQuery $query The query restricts the subscription charges which are returned by the search. (required)
	 * @throws \WeArePlanet\Sdk\ApiException
	 * @throws \WeArePlanet\Sdk\VersioningException
	 * @throws \WeArePlanet\Sdk\Http\ConnectionException
	 * @return \WeArePlanet\Sdk\Model\SubscriptionLedgerEntry[]
